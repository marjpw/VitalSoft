...
public class PanelDetalleLectura extends JPanel {
    private MainFrame mainFrame;
    private ServiceManager services;
    private Medico medicoLogueado;
    private Cita cita;

    public PanelDetalleLectura(MainFrame mainFrame, ServiceManager services, Medico medicoLogueado, Cita cita) {
        ...
        String peso = cita.isTriajeRealizado() ? String.valueOf(cita.getPeso()) : "-";
        String talla = cita.isTriajeRealizado() ? String.valueOf(cita.getTalla()) : "-";
        String presion = cita.isTriajeRealizado() ? cita.getPresionArterial() : "-";
        ...
        agregarSeccion(tarjeta, "Diagnóstico y Tratamiento:", 8);
        ...
        // Datos del médico que atendió LA CITA (no el logueado)
        JLabel lblDr = new JLabel(cita.getMedico().getNombre() + " " + cita.getMedico().getApellidos());
        lblDr.setFont(new Font("Century Gothic", Font.BOLD, 12));
        ...
        lblEsp.setFont(new Font("Century Gothic", Font.PLAIN, 10));
        ...
        btnRegresar.addActionListener(e -> mainFrame.mostrarPanelHistorialClinico(medicoLogueado, cita.getPaciente()));
    ...
    private void agregarCampo(JPanel p, String label, String valor, int x, int y) {
        JPanel panelCampo = creatingCampoPanel(label, valor);
        GridBagConstraints c = new GridBagConstraints();
        c.gridx = x;
        c.gridy = y;
    ...
    private JPanel crearCampoSimple(String label, String valor) {
        return creatingCampoPanel(label, valor);
    }

    private JPanel creatingCampoPanel(String label, String valor) {
        JPanel panelCampo = new JPanel(new BorderLayout(5, 0));
        panelCampo.setBackground(Color.WHITE);
        JLabel lbl = new JLabel(label);
        lbl.setPreferredSize(new Dimension(100, 20));
        panelCampo.add(lbl, BorderLayout.WEST);
