...
public class Menu {
    ...
    public static void mostrarMenuAdministrador(Administrador admin, ServiceManager services) {
        boolean salir = false;
        while (!salir) {
            System.out.println("\n===== PANEL ADMINISTRADOR =====");
            ...
            try {
                ...
                switch (op) {
                    case 1:
                        flujoRegistroMedico(services);
                        break;
                    case 2:
                        flujoRegistroPaciente(services);
                        break;
                    case 3:
                        System.out.print("ID Médico a eliminar: ");
                        services.getMedicoService().eliminarMedico(Integer.parseInt(sc.nextLine()));
                        break;
                    case 4:
                        System.out.println("-- Médicos --");
                        services.getMedicoService().verMedicos();
                        System.out.println("-- Pacientes --");
                        services.getPacienteService().verPacientes();
                        break;

                    case 5: // FLUJO TRIAJE
                        System.out.println("\n--- MÓDULO DE TRIAJE ---");
                        List<Cita> pendientesTriaje = services.getCitaService().getCitasPendientesTriaje();
                        if (pendientesTriaje.isEmpty()) {
                            ...
                            services.getCitaService().registrarTriaje(idT, peso, talla, temp, pres);
                        }
                        break;

                    case 6: // FLUJO CAJA
                        System.out.println("\n--- MÓDULO DE CAJA ---");
                        List<Cita> porCobrar = services.getCitaService().getCitasPendientesDePago();
                        if (porCobrar.isEmpty()) {
                            ...
                            services.getFacturaService().generarFactura(Integer.parseInt(sc.nextLine()));
                        }
                        break;

                    case 7:
                        services.getFacturaService().reporteFinanciero();
                        break;
                    case 0:
                        salir = true;
                        break;
    ...
    private static void flujoRegistroMedico(ServiceManager services) {
        ...
        services.getMedicoService().registrarMedico(n, a, e, d, p, di, h, pass);
    }

    // 3. FLUJOS PACIENTE Y MÉDICO
    public static void flujoRegistroPaciente(ServiceManager services) {
        ...
        String pass = sc.nextLine();
        services.getPacienteService().registrarPaciente(n, a, d, c, nac, al, pass);
    }

    public static void flujoCrearCita(Paciente p, ServiceManager services) {
        Set<String> esps = services.getMedicoService().obtenerEspecialidades();
        if (esps.isEmpty()) {
        ...
        List<Medico> meds = services.getMedicoService().obtenerMedicosPorEspecialidad(esp);
        if (meds.isEmpty()) {
        ...
        try {
            ...
            services.getCitaService().crearCita(p, m, f, mot);
        } catch (Exception e) {
            System.out.println("Error.");
        }
    }

    public static void menuPaciente(Paciente p, ServiceManager services) {
        boolean salir = false;
        while (!salir) {
            ...
            try {
                ...
                switch (op) {
                    case 1:
                        flujoCrearCita(p, services);
                        break;
                    case 2:
                        for (Cita c : services.getPacienteService().getHistorialPaciente(p.getId()))
                            System.out.println(c);
                        break;
                    case 3:
                        services.getPacienteService().verFacturasDePaciente(p);
                        break;
                    case 4:
                        services.getPacienteService().consultarDiagnostico(p);
                        break;
                    case 5:
                        List<Cita> l = services.getPacienteService().getHistorialPaciente(p.getId());
                        for (Cita c : l)
                            if (c.getEstado() == EstadoCita.PENDIENTE)
                                System.out.println(c);
                        System.out.print("ID a cancelar: ");
                        services.getCitaService().cancelarCita(Integer.parseInt(sc.nextLine()), p);
                        break;
                    case 0:
                        salir = true;
                        break;
    ...
    public static void menuMedico(Medico m, ServiceManager services) {
        boolean salir = false;
        while (!salir) {
            System.out.println("\n=== DR. " + m.getApellidos() + " ===");
            ...
            try {
                ...
                switch (op) {
                    case 1:
                        List<Cita> l = services.getCitaService().getCitasMedico(m.getId());
                        if (l.isEmpty())
                    ...
                    case 2:
                        ...
                        services.getCitaService().registrarDiagnostico(id, d, r);
                        System.out.println("Paciente enviado a Caja.");
                        break;
                    case 3:
                        System.out.print("ID Cita: ");
                        services.getCitaService().marcarInasistencia(Integer.parseInt(sc.nextLine()));
                        break;
                    case 0:
                        salir = true;
                        break;
